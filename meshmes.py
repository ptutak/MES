...
def loadData(fileName):
    ...
    for x in data:
        if isinstance(data[x],str):
            data[x]=eval(data[x])
        if isinstance(data[x],abc.Mapping):
            newDict={}
            for k,v in data[x].items():
                newDict[eval(k)]=eval(v)
            data[x]=newDict
    return data
...
class Grid:
    def __init__(self,B,H,nB,nH,t0=0,edges=None,x=0,y=0):
        self.nB=nB
        self.nH=nH
        self.B=B
        self.H=H
    ...
    def updateNodes(self,values,attr):
        ...
        for n in self.nodes:
            n[attr]=next(values)
    def setElemPhysParams(self,k,ro,c):
        if isinstance(k,abc.Sequence):
            k=iter(k)
        else:
            k=iter([k for i in range(len(self))])
        if isinstance(c,abc.Sequence):
            c=iter(c)
        else:
            c=iter([c for i in range(len(self))])
        if isinstance(ro,abc.Sequence):
            ro=iter(ro)
        else:
            ro=iter([ro for i in range(len(self))])
        for elem in self:
            elem.k=next(k)
            elem.ro=next(ro)
            elem.c=next(c)
